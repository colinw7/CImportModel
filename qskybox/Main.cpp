...
main(int argc, char **argv)
{
  ...
  bool        reflection_map = false;
  bool        refraction_map = false;
  ...
  for (int i = 1; i < argc; ++i) {
    if (argv[i][0] == '-') {
      ...
      else if (arg == "reflection_map") {
        reflection_map = true;
      }
      else if (arg == "refraction_map") {
        refraction_map = true;
      }
  ...
  auto *app = new CQSkybox3D::App;

  app->setReflectionMap(reflection_map);
  app->setRefractionMap(refraction_map);
