...
class CGeomObject3D {
 ...
 public:
  CGeomObject3D(CGeomScene3D *pscene, const std::string &name) :
   pscene_(pscene), name_(name), selected_(false), visible_(true), draw_position_(true),
   coord_frame_(), position_(CPoint3D(0,0,0)), faces_(), lines_(), vertices_(),
   vertex_face_list_(), vertex_face_normal_(), view_matrix_(), dv_(0,0,0), da_(0,0,0) {
  }
  ...
  CGeomPoint3D &editPositionPoint() {
    return position_;
  }
  ...
  CGeomLine3D &getLine(uint i) const {
    return *lines_[i];
  }
  ...
  void setBasis(const CVector3D &right, const CVector3D &up,
                const CVector3D &dir);
  ...
  void spinX(double da) {
    da_.setX(da_.getX() + da);
  }

  void spinY(double da) {
    da_.setY(da_.getY() + da);
  }

  void spinZ(double da) {
    da_.setZ(da_.getZ() + da);
  }
