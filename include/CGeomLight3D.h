...
class CGeomLight3DData {
 private:
  ...
  CRGBA     diffuse_;
  CRGBA     specular_;
  CVector3D spot_direction_;
  double    spot_exponent_;
  double    spot_cutoff_;
  bool      directional_;
  double    constant_attenuation_;
  double    linear_attenuation_;
  double    quadratic_attenuation_;
 ...
 public:
  ...
  void setSpotCutOff   (double cutoff       ) {
    spot_cutoff_    = cutoff;
  }
  ...
  double getAttenuation(double dist) const {
    ...
    return 1.0/(constant_attenuation_ +
                dist*(linear_attenuation_ +
                      quadratic_attenuation_*dist));
  }

  double getSpotEffect(const CPoint3D &lpoint,
                       const CPoint3D &point) const {
