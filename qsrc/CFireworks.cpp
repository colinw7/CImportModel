...
CFireworks()
{
  ...
  injectData_.velocityXMin = -25;
  injectData_.velocityXMax =  25;
  injectData_.velocityYMin = 20;
  injectData_.velocityYMax = 100;
  ...
  config_->getValue("inject_x_velocity_min", "", &injectData_.velocityXMin);
  config_->getValue("inject_x_velocity_max", "", &injectData_.velocityXMax);
  config_->getValue("inject_y_velocity_min", "", &injectData_.velocityYMin);
  config_->getValue("inject_y_velocity_max", "", &injectData_.velocityYMax);
...
injectParticle()
{
  if (inject_count_ == 0) {
    ...
    particle->setPosition(origin_.x, origin_.y);

    particle->setVelocity(COSRand::randIn(injectData_.velocityXMin, injectData_.velocityXMax),
                          COSRand::randIn(injectData_.velocityYMin, injectData_.velocityYMax));
