...
vec4 gridColor(vec2 uv, vec2 camPos) {
  ...
  // calculate absolute distances to cell line centers for each lod and pick max X/Y to
  // get coverage alpha value
  ...
  vec4 c = lod2a > 0.0 ?
    gridColorThick : lod1a > 0.0 ? mix(gridColorThick, gridColorThin, lodFade) : gridColorThin;
