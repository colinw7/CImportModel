 ...
 public:
  ...
  void recalc() {
    ...
    if (getEqualScale()) {
      ...
      if (getScaleMin()) {
        if (std::fabs(factor_x1_) > std::fabs(factor_y1_)) {
          if (factor_x1_ < 0)
            factor_x1_ = -std::fabs(factor_y1_);
          else
            factor_x1_ =  std::fabs(factor_y1_);
        }
        else {
          if (factor_y1_ < 0)
            factor_y1_ = -std::fabs(factor_x1_);
          else
            factor_y1_ =  std::fabs(factor_x1_);
        }
      }
  ...
  void windowToPixel(double window_x, double window_y, double *pixel_x, double *pixel_y) const {
    if (getEqualScale()) {
      if (flip_y_) {
        ...
        *pixel_x = (window_x - window1_.xmin)*factor_x1_ + pixel_.xmin + pdx_;
        *pixel_y = (window_y - window1_.ymin)*factor_y1_ + pixel_.ymin + pdy_;
      }
    }
    else {
    ...
    if (getEqualScale()) {
      ...
      if (factor_y1_ != 0.0) {
        if (flip_y_)
          ...
          *window_y = (pixel_y - pixel_.ymin - pdy_)/factor_y1_ + window1_.ymin;
      }
      else
