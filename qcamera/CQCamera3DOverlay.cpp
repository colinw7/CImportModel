...
shaderProgram()
{
  return getShader("overlay.vs", "overlay.fs");
}
...
drawGeometry()
{
  ...
  auto ph = canvas_->pixelHeight();

  auto os = 256;

  glViewport(pw - os, ph - os, os, os);
  ...
  //program->setUniformValue("viewPos", CQGLUtil::toVector(camera->viewRotationPosition()));
  auto viewPos = viewMatrix*CPoint3D(5, 5, 5);

  program->setUniformValue("viewPos",
    CQGLUtil::toVector(CGLVector3D(viewPos.x, viewPos.y, viewPos.z)));
